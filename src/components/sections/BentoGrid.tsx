...
const MarketplaceUI = () => (
  <div className="absolute top-1/2 right-12 -translate-y-1/2 w-[320px] bg-[#1C1C1A] rounded-3xl border-[4px] border-[#2A2A27] shadow-2xl overflow-hidden hidden lg:flex flex-col">
    {/* Header */}
    <div className="p-4 border-b border-white/5 bg-[#131312] flex items-center justify-between">
      <span className="text-xs font-inter font-bold text-white tracking-widest uppercase">Local Hub Fulfillment</span>
      <span className="text-[10px] text-[#6B8F5E] bg-[#6B8F5E]/10 px-2 py-1 rounded-full">Bharatpur Hub</span>
    </div>
    
    {/* Product List */}
    <div className="p-4 space-y-3 bg-[#131312] flex-1">
      {[
        { name: "Hybrid Maize Seeds", type: "Rampur Hybrid-10", price: "रू850", stock: "In Stock" },
        { name: "Organic NPK Blend", type: "50kg Sack", price: "रू2,100", stock: "2 left" }
      ].map((item, i) => (
        <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
           <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#6B8F5E]/20 to-[#C4704A]/20 flex items-center justify-center border border-white/10">
                <Plus className="w-4 h-4 text-[#A3A3A3]" />
             </div>
             <div>
               <p className="text-xs font-bold text-white">{item.name}</p>
               <p className="text-[9px] text-[#A3A3A3] mt-0.5">{item.type}</p>
             </div>
           </div>
           <div className="text-right">
              <p className="text-xs font-outfit text-[#C4704A]">{item.price}</p>
              <p className="text-[9px] text-[#6B8F5E] mt-0.5">{item.stock}</p>
           </div>
        </div>
      ))}
    </div>
    ...
    <div className="p-4 bg-background border-t border-white/5">
      <div className="w-full h-10 rounded-xl bg-[#6B8F5E] flex items-center justify-center text-white text-[10px] font-bold shadow-[0_0_20px_rgba(107,143,94,0.3)]">
        Direct to Distributor Sale
      </div>
    </div>
  </div>
);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:auto-rows-[450px]">
            ...
            <div className="h-full flex flex-col sm:flex-row gap-8 items-center sm:items-stretch pt-12 sm:pt-0">
              <div className="flex-1 flex flex-col justify-end relative z-10">
                ...
                <p className="text-[#A3A3A3] font-inter text-sm md:text-base leading-relaxed max-w-sm">
                  Powered by <b>Plant.id</b> and <b>Gemini 1.5 Flash</b>. Our localized vision models identify 100+ native diseases with 99% accuracy, providing family-specific advisory via RAG.
                </p>
              </div>
          ...
          >
            ...
            <div className="mt-4">
              ...
              <p className="text-[#A3A3A3] font-inter text-xs leading-relaxed">High-precision hyper-local forecasting powered by <b>ECMWF</b> models, optimized for Nepal's valley dynamics.</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="pt-10 h-full flex flex-col justify-between">
              <SoilNPK />
              <div className="mt-6">
                <h3 className="text-2xl font-outfit text-white mb-2">Soil Health</h3>
                <p className="text-[#A3A3A3] font-inter text-xs leading-relaxed">Real-time tracking of NPK levels and health via <b>Sentinel-2</b> satellite multispectral data.</p>
              </div>
            </div>
          </motion.div>
            ...
            <div className="h-full flex flex-col justify-end relative z-10 pt-16 sm:pt-0">
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 w-full">
                <div className="max-w-xs">
                  <h3 className="text-3xl font-outfit text-white mb-3 underline decoration-[#6B8F5E]/30 decoration-2 underline-offset-8">Satellite Mapping</h3>
                  <p className="text-[#A3A3A3] font-inter text-sm leading-relaxed">
                    Utilizing <b>Google Earth Engine (GEE)</b> to map field boundaries and analyze <b>NDVI</b> biomass health with sub-meter precision.
                  </p>
                </div>
          ...
          {/* Card 5: Marketplace (Row 3, Full Width) */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            whileInView={{ opacity: 1, scale: 1 }}
            viewport={{ once: true }}
            className="md:col-span-3 bg-[#232321] rounded-3xl p-8 md:p-12 relative overflow-hidden group border border-white/10"
          >
            <FeatureTag text="Marketplace" type="terracotta" />
            
            {/* Background gradient */}
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#C4704A]/5 to-transparent blur-[80px] pointer-events-none" />

            <MarketplaceUI />
            
            <div className="h-full flex flex-col justify-center relative z-10 max-w-lg">
              <h3 className="text-3xl md:text-5xl font-outfit text-white mb-4 tracking-tight leading-tight">
                Seed to Sale. <br />
                <span className="text-[#C4704A] italic">Zero middlemen.</span>
              </h3>
              <p className="text-[#A3A3A3] font-inter text-sm md:text-base leading-relaxed mb-8">
                Procure high-quality seeds and fertilizers from verified Local Hubs. 
                When harvest time comes, connect directly with wholesale distributors to sell your yield at transparent market rates.
              </p>
              
              <div className="flex gap-4">
                 <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex-1">
                    <span className="text-xl font-outfit text-white block mb-1">Local Hubs</span>
                    <span className="text-[10px] text-[#A3A3A3] uppercase tracking-widest">Input Procurement</span>
                 </div>
                 <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex-1">
                    <span className="text-xl font-outfit text-white block mb-1">Direct Sales</span>
                    <span className="text-[10px] text-[#A3A3A3] uppercase tracking-widest">Wholesale Connection</span>
                 </div>
              </div>
            </div>
          </motion.div>
